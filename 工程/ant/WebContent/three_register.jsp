...
<body>
	...
	<div class="wrapper">
		...
		<div class="article">
			<div class="nav">
				<div class="top_tag nav_person active">
					<span onClick="personReg()">个人注册</span>
				</div>
				<div class="top_tag nav_class" onClick="classReg()">
					<span>班委注册</span>
				</div>
				<div class="top_tag nav_org" onClick="orgReg()">
					<span>组织注册</span>
				</div>
			</div>
			<div class="clear"></div>
				...
				<form action="studentRegister" method="post">
					...
					<div class="normal">
						<input id="psd" type="password" name="password"
							placeholder="密码6~18位，以字母开头，只能包含字母、数字和下划线"
							value="${student.password }">
						<p id="psd_msg"></p>
					</div>
					<div class="normal">
						<input id="againpsd" type="password" name="againpwd"
							placeholder="确认密码" value="${againpsd }">
						<p id="againpsd_msg"></p>
					</div>
					<label class="sex"> <span class="sex_span">性别</span>
					</label> <label class="sex_man"> <input type="radio" name="gender"
						checked value="男">男
					</label> <label class="sex_woman"> <input type="radio"
			...
			<div class="reg hidden">
				<form>
					<div class="select_college">
						<select id="college" name="college">
							...
							<c:forEach items="${college }" var="col">
								<option value="${col.name }">${col.name }<c:if
										test="${col.name==collegeSelected}">selected</c:if></option>
							</c:forEach>
						</select>
						<p id="college_msg"></p>
					</div>
						...
						<select id="profession" name="profession">
							...
							<c:forEach items="${college }" var="col">
								<c:forEach items="${col.professions }" var="pro">
									<option value="${pro.name }">${pro.name }<c:if
											test="${pro.name==collegeSelected}">selected</c:if></option>
								</c:forEach>
							</c:forEach>
						</select>
					...
					<div class="select_normal">
						<select id="grade" name="grade">
							...
							<c:forEach items="${grade }" var="gra">
								<option value="${gra }">${gra }级<c:if
										test="${gra==gradeSelected}">selected</c:if></option>
							</c:forEach>
						</select>
						<p id="grade_msg"></p>
					</div>
					<div class="select_normal">
						<select id="classes" name="classes">
							...
							<c:forEach items="${classes }" var="cla">
								<option value="${cla }">${cla }班<c:if
										test="${cla==classesSelected}">selected</c:if></option>
							</c:forEach>
						</select>
						<p id="classes_msg"></p>
					</div>
					<div class="normal">
						<input id="mon_psd" type="password" name="password"
							placeholder="6-16位密码，区分大小写">
						<p id="mon_psd_msg"></p>
					</div>
					<div class="normal">
						<input id="mon_againpsd" type="password" name="pwd"
							placeholder="确认密码">
					...
					<div class="regist">
						<input type="submit" class="submit" value="注册"> <a href="">已有账户？登录</a>
					</div>
				</form>
			</div>
			<div class="reg hidden">
				<form>
					...
					<div class="normal">
						<input id="org_psd" type="password" name="password"
							placeholder="6-16位密码，区分大小写">
						<p id="org_psd_msg"></p>
					</div>
					<div class="normal">
						<input id="org_againpsd" type="password" name="pwd"
							placeholder="确认密码">
