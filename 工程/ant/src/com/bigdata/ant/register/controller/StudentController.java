...
import com.bigdata.ant.register.service.StudentServiceImpl;
...
public class StudentController {
	...
	public String toRegister(HttpServletRequest request) {
		request.setAttribute("college", studentServiceImpl.findCollege());// 把学院信息查出来
		request.setAttribute("grade", IncreaseTimeUtil.addDateYear(4));// 把年级信息查出来
		request.setAttribute("classes", studentServiceImpl.findClasses());// 把班级信息列出来
	...
	public void checkStudentId(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		Boolean bool = studentServiceImpl.findStudentById(student_id);
	...
	public String studentRegister(HttpServletRequest request, Student student) throws IOException {
		...
		String admitStudentRegister = studentServiceImpl.admitStudentRegister(student, againpsd);// 获得信息（是否允许注册）
		if (admitStudentRegister.equals("0")) {// 该用户注册成功，待激活
			studentServiceImpl.processRegister(student);
	...
	public String activeStudent(HttpServletRequest request, HttpServletResponse response) {
		...
		String msg = studentServiceImpl.VolidateRegister(email, validateCode);
